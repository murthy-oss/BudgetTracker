...
// Example deletion function
  ...
  void initState() {
    ...
    //BlocProvider.of<HomeBloc>(context).add(TotalAmountCalculateEvent(totalAmount));
    //read_data();
  ...
  Widget build(BuildContext context) {
    return BlocConsumer<HomeBloc, HomeState>(
      ...
      listener: (context, state)  {
        if (state is AddNewElementState) {
          ...
          _homeBloc.add(HomeListOfItemsFromDateEvent(_selectedDate, expenses));
        } else if (state is TotalAmountCalculateState) {
          setState(() {
            totalAmount = state.totalAmount;
            print('ekkada$totalAmount');
          });
        }
        else if(state is HomeListOfItemsFromDateState){
          setState(() {
            Expenses=state.expenses;
            print('edhe$Expenses');
          });
        }
        else if(state is TotalAmontCaluculatingSate){
          LoadingAnimationWidget.waveDots(
                             color: Colors.black,
                              size: 30,
                        );
        }
        else if(state is DeleteItemFromHomeListState){
        ...
        else if(state is EditButtonNavigationState){
print("Success Ok");
           Navigator.push(
                                                                context,
                                                                MaterialPageRoute(
      ...
      buildWhen: (previous, current) => current is TotalAmontCaluculatingSate || current is TotalAmountCalculateState || current is HomeListOfItemsFromDateState,
      bloc: _homeBloc,
      builder: (context, state) {
        if( state is TotalAmountCalculateState || state is HomeListOfItemsFromDateState){
          //final datastate=state as HomeListOfItemsFromDateState;
        ...
        return Scaffold(
          ...
          floatingActionButton: FloatingActionButton.extended(
              ...
              onPressed: _isFabVisible
                  ? () {
                      _homeBloc.add(AddNewElementEvent());
                      /*   setState(() {
                                                ...
                                                Row(
                                                  ...
                                                  children: [
                                                    ...
                                                    IconButton(
                                                        onPressed: () async {
                                                          print(expense.Id);
                                            _homeBloc.add(DeleteItemFromHomeListEvent(expense.Id));
                                                          // await deleteExpense(
                                                          //     expense.Id);
                                                          // Navigator.push(
                                                          //     context,
